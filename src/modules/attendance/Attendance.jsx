...
export default function Attendance({ 
  ...
  // --- 1. LOAD METADATA ---
  useEffect(() => {
    if (projectId && eventId && profile) loadMetadata();
  ...
  const markAttendance = async (memberId) => {
    ...
    setPresentMap(prev => {
        ...
        if (isPresent) next.delete(memberId); else next.set(memberId, now);
        return next;
    });
    ...
    try {
        ...
        setPresentMap(prev => {
            const next = new Map(prev);
            if (isPresent) next.set(memberId, now); else next.delete(memberId);
            return next;
        });
  ...
  const handleScan = async (code) => {
    ...
    const member = members.find(m => m.internal_code === code || m.id === code);
  ...
  return (
    <div className={`flex flex-col bg-slate-50 ${embedded ? 'h-full rounded-xl border border-slate-200 shadow-sm overflow-hidden' : 'h-[100dvh]'}`}>
      
      {/* HEADER */}
      <div className="bg-white border-b border-slate-200 sticky top-0 z-10 px-4 py-3 shadow-sm">
        <div className="flex items-center gap-3 mb-3">
          {!embedded && <button onClick={() => navigate(-1)} className="p-2 -ml-2 rounded-full hover:bg-slate-100"><ArrowLeft size={22}/></button>}
          
          <div className="flex-1 min-w-0">
             <h1 className="font-bold text-lg text-slate-800 leading-tight truncate">{event.name}</h1>
             <div className="flex items-center gap-2 text-xs text-slate-500 mt-0.5">
               <span className={`w-2 h-2 rounded-full ${syncing ? 'bg-yellow-400 animate-pulse' : 'bg-green-500'}`}></span>
               <span className="font-semibold text-slate-700">{presentCount}</span> / {members.length} Present
               {!canMark && <span className="ml-2 px-1.5 py-0.5 bg-slate-100 text-slate-500 rounded text-[10px] uppercase font-bold flex items-center gap-1"><Lock size={8}/> View</span>}
             </div>
          </div>

          <div className="flex gap-2">
            {!hideSummary && (
              <button onClick={() => setShowSummary(true)} className="bg-white text-indigo-600 border border-indigo-100 p-3 rounded-xl shadow-sm active:scale-95 transition-transform">
                <BarChart2 size={20} />
              </button>
            )}
            {canMark && (
              <button onClick={() => setIsScannerOpen(true)} className="bg-indigo-600 text-white p-3 rounded-xl shadow-lg shadow-indigo-200 active:scale-95 transition-transform">
                <QrCode size={20} />
              </button>
            )}
          </div>
        ...
        <div className="flex gap-2">
            <div className="relative flex-1">
                ...
                <input className="w-full pl-10 pr-4 py-2 bg-slate-100 border-transparent focus:bg-white focus:border-indigo-500 border rounded-xl outline-none text-sm" placeholder="Search..." value={search} onChange={e => setSearch(e.target.value)}/>
            </div>
            <div className="flex bg-slate-100 p-1 rounded-xl shrink-0">
               {['all', 'present', 'absent'].map(f => (
                 <button key={f} onClick={() => setFilter(f)} className={`px-3 py-1.5 text-xs font-bold capitalize rounded-lg transition-all ${filter === f ? 'bg-white text-indigo-600 shadow-sm' : 'text-slate-500'}`}>{f}</button>
               ))}
            </div>
        </div>
        ...
        {mandalFilter && (
          <div className="mt-2 flex items-center justify-between bg-indigo-50 px-3 py-2 rounded-lg text-xs text-indigo-700 font-bold border border-indigo-100 animate-in slide-in-from-top-2">
            <span className="flex items-center gap-1"><Filter size={12}/> Filtering: {mandalFilterName}</span>
            <button onClick={() => {setMandalFilter(null); setMandalFilterName(null)}} className="p-1 hover:bg-indigo-100 rounded-full"><X size={14}/></button>
          </div>
        )}
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-2 pb-20 relative">
         ...
         {error && (
            <div className="p-6 text-center bg-red-50 rounded-xl border border-red-100">
               ...
               <button onClick={loadRosterData} className="mt-3 text-xs bg-white border border-red-200 px-3 py-1.5 rounded-lg text-red-700 font-bold flex items-center gap-1 mx-auto">
            ...
            return (
                ...
                >
                    <div className="flex items-center gap-3 overflow-hidden">
                         <div className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm shrink-0 ${isPresent ? 'bg-green-500 text-white shadow-md shadow-green-200' : 'bg-slate-100 text-slate-500'}`}>
                             {isPresent ? <Check size={20}/> : m.name[0]}
                         </div>
                         <div className="min-w-0">
                             <div className={`font-bold text-sm truncate ${isPresent ? 'text-green-800' : 'text-slate-800'}`}>{m.name} {m.surname}</div>
                             <div className="text-xs text-slate-500 truncate">{m.mandal} • {m.designation}</div>
                         </div>
                    </div>
                    {isPresent && <span className="text-[10px] font-bold bg-green-100 text-green-700 px-2 py-1 rounded-full">IN</span>}
                </div>
            );
         })}
      </div>
