...
import {
  Loader2, Calendar, FolderKey, Clock, QrCode, ArrowRight,
  LayoutDashboard, AlertTriangle, RefreshCw, CheckCircle2,
} from "lucide-react";
...
export default function HomeDashboard() {
  ...
  const { data, isLoading, isError, refetch } = useQuery({
    queryKey: ["home-dashboard", profileId],
    queryFn: async () => {
      let allowedProjectIds = [];

      if (!isAdmin) {
        const { data: assignments, error: aErr } = await withTimeout(
          supabase.from("project_assignments").select("project_id").eq("user_id", profileId)
        );
        const assignedIds = assignments?.map((a) => a.project_id) || [];

        let standardIds = [];
        if (role !== "project_admin") {
          const { data: standardProjects, error: sErr } = await withTimeout(
        ...
        if (allowedProjectIds.length === 0)
          return { primaryEvent: null, primaryProject: null, openRegistrations: [], recentProjects: [] };
      }
      ...
      const { data: eventsData, error: eErr } = await withTimeout(eventQuery);

      let primaryEvent = eventsData?.[0] || null;
      let primaryProject = primaryEvent
        ? Array.isArray(primaryEvent.projects) ? primaryEvent.projects[0] : primaryEvent.projects
        : null;

      if (primaryProject && !isAdmin &&
        primaryProject.allowed_gender !== "Both" &&
        primaryProject.allowed_gender !== profile?.gender) {
        primaryEvent = null;
        primaryProject = null;
      }
      ...
      const { data: openRegistrations, error: rErr } = await withTimeout(regQuery);

      const validRegs = (openRegistrations || []).filter(
        (p) => isAdmin || p.allowed_gender === "Both" || p.allowed_gender === profile?.gender
      );
      ...
      const { data: recentProjects, error: pErr } = await withTimeout(projQuery);

      const validRecent = (recentProjects || []).filter(
        (p) => isAdmin || p.allowed_gender === "Both" || p.allowed_gender === profile?.gender
      );
  ...
  if (profile && !profileId) {
    return (
      <div className="p-12 text-center bg-amber-50 rounded-md border border-amber-200 mt-10 max-w-lg mx-auto">
        <AlertTriangle className="mx-auto text-amber-400 mb-3" size={32} strokeWidth={1.5} />
        <h3 className="text-gray-900 font-bold mb-1">Profile configuration error</h3>
        <p className="text-gray-500 text-sm mb-2">Profile loaded but ID field not found.</p>
        <p className="text-xs font-mono bg-white border border-amber-200 rounded p-2 text-left break-all">
      ...
      <div className="h-[calc(100vh-80px)] flex flex-col">
        <div className="mb-4">
          <h1 className="text-2xl font-bold text-gray-900 tracking-tight">
            Jay Swaminarayan, {profile.full_name?.split(" ")[0]}
          </h1>
          ...
          <div className="bg-white border border-gray-200 rounded-xl shadow-[0_1px_3px_rgba(0,0,0,0.02)] overflow-hidden">
            {openRegistrations?.length === 0 ? (
              <div className="p-8 text-center text-gray-500 text-sm">
                No projects are currently accepting registrations.
              </div>
            ) : (
                  ...
                  >
                    <div className="min-w-0 pr-4">
                      ...
                      <div className="text-[10px] text-gray-500 font-medium mt-0.5 flex items-center gap-2">
                        <Badge variant={p.type === "Restricted" ? "danger" : "default"} className="!text-[9px] !py-0">
                          {p.type}
                        </Badge>
