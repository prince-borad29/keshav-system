...
export default function ProjectStaff({ project, isAdmin, isCoordinator }) {
    ...
    try {
      ...
      if (existingProfile) {
        ...
        const { data: existingAssign } = await supabase
          ...
          .maybeSingle();
        if (existingAssign)
          throw new Error(`${selectedMember.name} is already assigned!`);
      } else {
      ...
      const { error: insertError } = await supabase
        ...
        .insert({
          ...
          scope_mandal_ids: dataScope === "Mandal" ? [selectedMember.mandal_id] : null,
        });
  ...
  const handleUpdateAccess = async (e) => {
    ...
    try {
      const { data, error } = await supabase
        .from("project_assignments")
        .update({ role: projectRole, data_scope: dataScope })
      ...
      if (!data || data.length === 0) {
        throw new Error("Update failed! The database blocked the save. Check your Row Level Security (RLS) policies.");
      }
  ...
  if (isLoading)
    return (
      <div className="p-12 text-center text-gray-400">
        <Loader2 className="animate-spin inline mr-2" size={20} /> Loading staff...
      </div>
    );
  ...
  return (
    <div className="space-y-6">
      {canManageStaff && (
        <div className="bg-white p-4 rounded-md border border-gray-200 shadow-[0_1px_3px_rgba(0,0,0,0.02)] relative z-20">
          <label className="block text-sm font-bold text-gray-900 mb-1 flex items-center gap-2">
            <UserPlus size={16} className="text-gray-400" /> Add Project Staff
          </label>
          <p className="text-xs text-gray-500 mb-3">
            Search directory. App logins will be generated automatically if needed.
          </p>
          ...
          <div className="relative">
            ...
            {searching && (
              <Loader2 className="absolute right-3 top-2.5 text-gray-400 animate-spin" size={16} strokeWidth={1.5} />
            )}
          </div>
          ...
          {memberResults.length > 0 && (
            <div className="absolute left-4 right-4 mt-1 border border-gray-200 rounded-md shadow-[0_4px_20px_rgba(0,0,0,0.08)] bg-white overflow-hidden max-h-60 overflow-y-auto z-50 divide-y divide-gray-100">
              {memberResults.map((m) => (
                <div
                  key={m.id}
                  className="p-3 hover:bg-gray-50 flex items-center justify-between cursor-pointer transition-colors"
                  onClick={() => {
                    setProjectRole("volunteer");
                    setDataScope("Mandal");
                    setSelectedMember(m);
                    setMemberResults([]);
                    setSearchTerm("");
                ...
                >
                  <div>
                    <div className="font-semibold text-gray-900 text-sm">
                      {m.name} {m.surname} <Badge variant="default" className="ml-2">{m.designation}</Badge>
                    </div>
                    <div className="text-xs text-gray-500 mt-0.5 flex items-center gap-2">
                      <MapPin size={12} /> {m.mandals?.name} <span className="font-inter">({m.internal_code})</span>
                    </div>
                  </div>
            ...
            {assignments.length === 0 ? (
              <tr>
                <td colSpan={3} className="p-8 text-center text-gray-400 text-sm">
                  No custom staff assigned. Admins have global access.
                </td>
              </tr>
            ) : (
              assignments.map((a) => (
                <tr key={a.id} className="hover:bg-gray-50 transition-colors group">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-gray-500 flex items-center gap-1 mt-0.5">
                          <Mail size={10} strokeWidth={2} /> {a.user_profiles?.email}
                        </div>
                      </div>
                    </div>
                  </td>
                  <td className="px-4 py-3">
                    <Badge
                      variant={a.role?.toLowerCase() === "coordinator" ? "primary" : "default"}
                  ...
                  <td className="px-4 py-3 text-right">
                    {canManageStaff && (
                      <div className="flex justify-end gap-1 transition-opacity">
                        <button
                          onClick={() => {
                            setProjectRole(a.role || "volunteer");
                            setDataScope(a.data_scope || "Mandal");
      ...
      <Modal
        ...
        title={`Configure Role: ${selectedMember?.name}`}
      >
        <form onSubmit={handleGrantAccess} className="space-y-4">
          ...
          <div className="pt-4 border-t border-gray-100 flex justify-end gap-2">
            ...
            <Button type="submit" disabled={assigning}>
              {assigning ? <Loader2 className="animate-spin" size={16} /> : "Grant Access"}
            </Button>
          </div>
        </form>
      </Modal>
      ...
      <Modal
        ...
        title={`Edit Role: ${editingAssignment?.user_profiles?.full_name}`}
      >
        <form onSubmit={handleUpdateAccess} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelClass}>Project Role</label>
              {/* 🛡️ FIX: Updated to match ENUM exactly */}
              <select className={inputClass} value={projectRole} onChange={(e) => setProjectRole(e.target.value)}>
                <option value="volunteer">Volunteer</option>
                <option value="Editor">Editor</option>
                <option value="Coordinator">Coordinator</option>
          ...
          <div className="pt-4 border-t border-gray-100 flex justify-end gap-2">
            ...
            <Button type="submit" disabled={assigning}>
              {assigning ? <Loader2 className="animate-spin" size={16} /> : "Save Changes"}
            </Button>
          </div>
        </form>
      </Modal>
          ...
          <div className="bg-red-50 text-red-800 p-4 rounded-md border border-red-100 flex gap-3">
            ...
            <p className="text-sm">
              ...
              <span className="font-bold">
                {assignmentToRemove?.user_profiles?.full_name}
              </span>
              ? They will no longer be able to manage this project.
            </p>
          </div>
        ...
        {newCredentials && (
          <div className="space-y-4 text-center">
            ...
            <p className="text-gray-500 text-sm">
              A new app account was generated for this staff member.
            </p>
            <div className="bg-gray-50 p-4 rounded-md border border-gray-200 text-left space-y-3">
              <div>
                ...
                <div className="font-inter font-bold text-gray-900 bg-white border border-gray-200 p-2 rounded-md">
                  {newCredentials.email}
                </div>
              </div>
              <div>
                <span className={labelClass}>Temporary Password</span>
                <div className="font-inter font-bold text-[#5C3030] bg-[#5C3030]/10 border border-[#5C3030]/20 p-2 rounded-md">
