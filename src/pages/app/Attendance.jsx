...
import { 
  ...
  Filter, SortAsc, Plus, ChevronDown, GripVertical, ArrowUp, ArrowDown, Trash2
} from 'lucide-react';
  ...
  const fetchData = async () => {
    ...
    if (regData && regData.length > 0) {
       ...
       const { data: memData } = await supabase
         .from('members')
         .select('id, name, surname, mandal, mobile_number, designation')
         .in('id', ids)
         .order('name');
                 ...
                 <div className="flex items-center gap-2">
                    ...
                    {isPresent ? (
                      <button 
                        ...
                        className="px-4 py-2 bg-green-100 text-green-700 font-bold rounded-lg text-sm border border-green-200 hover:bg-green-200 transition-colors min-w-[90px]"
                      >
                        Present
                      </button>
                    ) : (
                      <button 
                        onClick={() => toggleAttendance(m.id)}
                        className="px-4 py-2 bg-slate-100 text-slate-500 font-bold rounded-lg text-sm border border-slate-200 hover:bg-slate-200 transition-colors min-w-[90px]"
                      >
                        Absent
                      </button>
                    )}
                  ...
                  {filter.column && (
                    <div className="relative">
                      <MultiSelectDropdown 
                        ...
                        onChange={(newValues) => updateFilterRow(filter.id, 'values', newValues)} 
                      />
                    </div>
                  )}
                </div>
              ))}
              <button onClick={addFilterRow} className="w-full py-3 border-2 border-dashed border-slate-300 rounded-xl text-slate-500 font-bold hover:bg-white transition-all flex items-center justify-center gap-2">
            ...
            <div className="space-y-3">
              {localSorts.map((sort) => (
                <div key={sort.id} className="bg-white p-3 rounded-xl border border-slate-200 shadow-sm flex items-center gap-2">
                   ...
                   <button onClick={() => updateSortRow(sort.id, 'asc', !sort.asc)} className="p-2 bg-slate-50 border border-slate-200 rounded-lg text-slate-600 font-bold text-xs min-w-[60px] flex flex-col items-center justify-center">
                     {sort.asc ? <ArrowUp size={14} className="text-sky-600 mb-1"/> : <ArrowDown size={14} className="text-orange-500 mb-1"/>}
                     {sort.asc ? 'ASC' : 'DESC'}
                   </button>
                   <button onClick={() => removeSortRow(sort.id)} className="p-2 text-slate-400 hover:text-red-500"><Trash2 size={18} /></button>
                </div>
              ))}
              <button onClick={addSortRow} className="w-full py-3 border-2 border-dashed border-slate-300 rounded-xl text-slate-500 font-bold hover:bg-white transition-all flex items-center justify-center gap-2">
