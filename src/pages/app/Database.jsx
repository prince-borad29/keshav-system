...
import { 
  Plus, Search, Phone, MoreVertical, X, Filter, SortAsc, Check, 
  ChevronDown, Trash2, ArrowUp, ArrowDown, GripVertical, Users
} from 'lucide-react';
import { supabase } from '../../lib/supabase';
import MemberModal from '../../components/MemberModal';
import ConfirmModal from '../../components/ConfirmModal'; // ✅ IMPORT

// ... (Keep CONSTANTS: DESIGNATION_HIERARCHY, ALL_COLUMNS) ...
...
export default function Database() {
  ...
  const fetchMembers = async () => {
    setLoading(true);
    try {
      const { data: membersData, error: memberError } = await supabase.from('members').select('*');
  ...
  // ... (Keep getVisibleDesignationChips, useEffect for Filter/Sort engine, openDrawer, getGroupedMembers) ...
  // [Paste the logic from previous Database.jsx here if needed, keeping it brief for clarity]
  
  // --- ENGINE ---
  const getVisibleDesignationChips = () => {
    const existing = new Set(members.map(m => m.designation));
    const available = DESIGNATION_HIERARCHY.filter(d => existing.has(d));
  ...
  const sortedMandalKeys = Object.keys(groupedMembers).sort();
    ...
    <div className="flex flex-col h-full bg-slate-50 relative overflow-hidden" onClick={() => setActiveMenuId(null)}>
      ...
      <div className="bg-white p-4 pb-2 shadow-sm z-10 sticky top-0 pt-safe-top">
        ...
        <div className="relative mb-3">
           ...
           <input type="text" placeholder="Search Name, Mobile, ID..." value={search} onChange={e => setSearch(e.target.value)} className="w-full pl-10 pr-10 py-3 bg-slate-50 border border-slate-200 rounded-xl outline-none focus:border-[#002B3D]" />
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
          {getVisibleDesignationChips().map(d => {
            const count = d === 'All' ? members.length : members.filter(m => m.designation === d).length;
            return (
              <button key={d} onClick={() => setQuickDesigFilter(d)} className={`px-4 py-1.5 rounded-full text-sm font-semibold whitespace-nowrap transition-colors border flex items-center gap-2 ${quickDesigFilter === d ? 'bg-[#002B3D] text-white border-[#002B3D]' : 'bg-white text-slate-500 border-slate-200 hover:bg-slate-50'}`}>
      ...
      <div className="flex-1 overflow-y-auto p-4 pb-24 space-y-6 pb-safe-bottom">
        {loading ? <div className="text-center text-slate-400 mt-10">Loading...</div> : (
           sortedMandalKeys.length === 0 ? <div className="text-center text-slate-400 mt-10">No members found.</div> : 
             sortedMandalKeys.map(mandal => (
               <div key={mandal} className="space-y-3">
                 <div className="flex items-center gap-2 px-1">
                    <h2 className="text-[#002B3D] font-bold text-lg">{mandal}</h2>
                    <span className="bg-slate-200 text-slate-600 text-xs px-2 py-0.5 rounded-full font-bold">{groupedMembers[mandal].length}</span>
                 </div>
                 {groupedMembers[mandal].map(m => (
                      ...
                      <div className="flex justify-between items-start">
                         <div>
                            <div className="flex items-center gap-2 mb-1">
                              ...
                              <span className="text-[10px] bg-sky-50 text-[#002B3D] border border-sky-100 px-1.5 py-0.5 rounded font-mono font-bold tracking-wider">{m.id}</span>
                            </div>
                            <p className="text-sm text-slate-500 font-medium mb-2">{m.designation} • {m.mobile_number || 'No Mobile'}</p>
                            <div className="flex flex-wrap gap-1">
                              {m.entity_tags?.map((et, i) => (
                                <span key={i} className="text-[10px] px-2 py-0.5 rounded-full font-bold text-white shadow-sm" style={{ backgroundColor: et.tags?.color || '#94a3b8' }}>{et.tags?.name}</span>
                              ))}
                            </div>
                         </div>
                         <div className="flex items-center gap-1">
                            {m.mobile_number && <a href={`tel:${m.mobile_number}`} className="p-2 text-[#0EA5E9] bg-sky-50 rounded-full hover:bg-sky-100 transition-colors"><Phone size={18} /></a>}
                            <button onClick={(e) => { e.stopPropagation(); setActiveMenuId(activeMenuId === m.id ? null : m.id); }} className="p-2 text-slate-400 hover:bg-slate-50 rounded-full transition-colors"><MoreVertical size={18} /></button>
                         </div>
                      </div>
                      {activeMenuId === m.id && (
                        <div className="absolute right-4 top-14 bg-white shadow-xl border border-slate-100 rounded-lg w-32 py-1 z-30 animate-in zoom-in-95 duration-100 origin-top-right">
                           <button onClick={() => { setMemberToEdit(m); setIsModalOpen(true); }} className="w-full text-left px-4 py-2 text-sm text-slate-700 hover:bg-slate-50 font-medium">Edit</button>
                           <button onClick={() => handleDelete(m.id)} className="w-full text-left px-4 py-2 text-sm text-red-500 hover:bg-red-50 font-medium">Delete</button>
                        </div>
                      )}
                   </div>
                 ))}
      ...
      <button onClick={() => { setMemberToEdit(null); setIsModalOpen(true); }} className="fixed bottom-6 right-6 w-14 h-14 bg-[#002B3D] text-white rounded-full shadow-lg flex items-center justify-center hover:bg-[#155e7a] hover:scale-105 transition-all z-20"><Plus size={28} /></button>
      ...
      <ConfirmModal 
        ...
        title="Delete Member?"
        message="This will permanently delete this member and their attendance history."
...
// ... (Keep RightDrawer & MultiSelectDropdown exactly as they are in Database.jsx) ...
// (I am omitting them here to save space, but DO NOT REMOVE them from your file)
// Copy them from your existing Database.jsx or ask me if you need them.
